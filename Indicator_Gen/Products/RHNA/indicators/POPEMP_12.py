...
if __name__ == '__main__':
    ...
    df_lodes = pd.read_excel(FILE_LODES, sheet_name='RAC')
    ...
    for year in tqdm(years):
        ...
        df['Year'] = year
        df['block_group'] = df['h_geocode'].astype('str')
        ...
        df = df.drop(['h_geocode', 'createdate', 'block_group', 'Geographic_Code_Identifier'], axis=1)
        df = df.groupby(['Year', 'COUNTY', 'JURIS'], as_index=False).sum()
        df = df.melt(id_vars=['Year', 'COUNTY', 'JURIS'], var_name='job_sector_code', value_name='Number of Job Holders')
        ...
        df = df.groupby(['Year', 'COUNTY', 'JURIS', 'Industry'], as_index=False)['Number of Job Holders'].sum()
