...
if __name__ == '__main__':
    ...
    df_places['Cost Burden'   ] = df_places['Housing Tenure'].apply(re_remove_pre )
    df_places['Housing Tenure'] = df_places['Housing Tenure'].apply(re_remove_post)
    df_places['Percent'] = df_places['Households'] / df_places.groupby(['County Name', 'Geography', 'Housing Tenure'])['Households'].transform('sum')

    df_places = df_places[['County Name', 'Geography', 'Cost Burden', 'Housing Tenure', 'Households', 'Percent']]
    df_places.loc[df_places['Cost Burden']=='50% or more of income used for housing', 'Cost Burden'] = '50%+ of income used for housing'

    df_places['Sort'] = pd.Categorical(df_places['Cost Burden'], ['0%-30% of income used for housing', '30%-50% of income used for housing', '50%+ of income used for housing', 'Not computed'])
    ...
    for county in counties:
        ...
        for jurisdiction in tqdm(jurisdictions, position=0):
            ...
            df_prod = df_places_sub[df_places_sub['Geography'] == jurisdiction].pivot(index='Cost Burden', columns='Housing Tenure', values='Households').reset_index()
            df_pct  = df_places_sub[df_places_sub['Geography'] == jurisdiction].pivot(index='Cost Burden', columns='Housing Tenure', values='Percent'   ).reset_index()
