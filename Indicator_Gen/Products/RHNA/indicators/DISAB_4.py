...
if __name__ == '__main__':
    ...
    df_cdds = df_cdds.melt(id_vars=['COUNTY', 'JURIS'], var_name='Age Group', value_name='Population')
    df_cdds['Population'] = df_cdds['Population'].str.replace(' ', '')
    df_cdds['Population'] = df_cdds['Population'].str.replace('>', '')
    df_cdds['Population'] = df_cdds['Population'].str.replace('<', '')
    df_cdds['Population'] = df_cdds['Population'].astype(int)

    df_cdds = df_cdds.groupby(['COUNTY', 'JURIS', 'Age Group'], as_index=False)['Population'].sum()
    ...
    df_cdds = df_cdds[df_cdds['Age Group'] != 'Total Age'].reset_index(drop=True)

    conditions = [df_cdds['Age Group'] == '00-17 yrs', df_cdds['Age Group'] == '18+ yrs']
    choices = ['Under 18', 'Over 18']
    ...
    for county in counties:
        ...
        for jurisdiction in tqdm(jurisdictions, position=0):
            ...
            df_prod = df_prod[['Age Group', 'Population']]
