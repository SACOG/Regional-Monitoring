...
if __name__ == '__main__':
    ...
    for file in tqdm(files):
        ...
        if file.stem == 'CoC_PopSub_CoC_CA-503-2024_CA_2024':
            df = df.rename(columns={'Summary of all other populations reported:':'Characteristic', 'Unnamed: 7':'Sheltered - Emergency Shelter', 'Unnamed: 9':'Sheltered - Transitional Housing', 'Unnamed: 13':'Unsheltered'})
        else:
            df = df.rename(columns={'Summary of all other populations reported:':'Characteristic', 'Unnamed: 7':'Sheltered - Emergency Shelter', 'Unnamed: 9':'Sheltered - Transitional Housing', 'Unnamed: 12':'Unsheltered'})

        df = df.dropna()
        str_to_keep = 'Severely Mentally Ill|Chronic Substance Abuse|HIV/AIDS|Veterans|Victims of Domestic Violence'
        df = df[df['Characteristic'].str.contains(str_to_keep)]
    ...
    for geography in geographies:
        ...
        for county in counties:
            ...
            for jurisdiction in tqdm(jurisdictions, position=0):
                ...
                df_plot = df_prod.melt(id_vars='Characteristic', var_name='Shelter Status', value_name='Number of Persons')
