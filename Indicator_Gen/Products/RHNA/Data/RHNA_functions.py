...
def split_notes(df):
    notes = dict_about[source][indicator_name.replace('RHNA_', '')]['Notes']
...
if source == 'DOF':
    ...
    def import_rhna(path_raw, indicator_name):
        
        workbooks = os.listdir(path_raw)
        workbooks = [workbook for workbook in workbooks if indicator_name in workbook]
        print(); print()
        print('Workbooks to import: ', workbooks)     

        path_places   = path_raw / f'{indicator_name} Places {source}.xlsx'  
        path_counties = path_raw / f'{indicator_name} Counties {source}.xlsx'
        path_mpo      = path_raw / f'{indicator_name} MPO {source}.xlsx'     
    ...
    def pivot_rhna(indicator_name, df_places_sub, county, jurisdiction, df_counties_sub=None, df_mpo=None):

        if indicator_name.replace('RHNA_', '') in ['POPEMP_1']:
            ...
            df_prod = df_prod.rename(columns = {'Population': jurisdiction, 'growth': f'Percent Difference {jurisdiction}'})
            df_prod = df_prod.merge(df_counties_sub, on='Year')
            df_prod = df_prod.merge(df_mpo         , on='Year')
            df_prod = df_prod[['Year', jurisdiction, f'{county} County', 'SACOG', f'Percent Difference {jurisdiction}', f'Percent Difference {county} County', 'Percent Difference SACOG']]
...
if source == 'ACS5':
    ...
    def import_rhna(path_raw, indicator_name):
        ...
        path_places   = path_raw / f'{indicator_name} Places ACS5.xlsx'  
        path_counties = path_raw / f'{indicator_name} Counties ACS5.xlsx'
        path_mpo      = path_raw / f'{indicator_name} MPO ACS5.xlsx'     
    ...
    def pivot_rhna(indicator_name, df_places_sub, county, jurisdiction, columns, values, df_counties_sub=None, df_mpo=None):

        if indicator_name.replace('RHNA_', '') in ['POPEMP_10', 'POPEMP_18', 'POPEMP_19', 'POPEMP_20', 'POPEMP_21', 'POPEMP_22', 'POPEMP_25', 'HSG_1', 'HSG_5', 'HSG_6'
        ...
        elif indicator_name.replace('RHNA_', '') in ['HSG_4', 'HSG_11', 'OVER_1', 'OVER_3', 'SEN_4', 'DISAB_1', 'DISAB_4', 'DISAB_5', 'ELI_3']:
            pass
        elif indicator_name.replace('RHNA_', '') in ['POPEMP_19']:
            pass
    ...
    ## export_rhna function will work for the following indicators:
    # POPEMP_3, POPEMP_5, POPEMP_6, POPEMP_7, POPEMP_8, POPEMP_9, POPEMP_16, POPEMP_23, POPEMP_24
    # HSG_2, HSG_3, HSG_7, HSG_9
    # RISK_1, OVER_2, OVER_7, FARM_1, LGFEM_2, DISAB_2, HOMELS_5, ELI_1, AFFH_3, HHPROJ_1
    style1 = ['POPEMP_3', 'POPEMP_5', 'POPEMP_6', 'POPEMP_7', 'POPEMP_8', 'POPEMP_9', 'POPEMP_16', 'POPEMP_23', 'POPEMP_24'
            , 'HSG_2', 'HSG_3', 'HSG_7', 'HSG_9'
            , 'RISK_1', 'OVER_2', 'OVER_7', 'FARM_1', 'LGFEM_2', 'DISAB_2', 'HOMELS_5', 'ELI_1', 'AFFH_3', 'HHPROJ_1']
    style2 = ['POPEMP_10', 'POPEMP_19', 'POPEMP_20']
    style3 = ['HSG_4']
...
if source == 'CHAS':
    ...
    def pivot_rhna(indicator_name, df_places_sub, county, jurisdiction, columns, values, df_counties_sub=None, df_mpo=None):
        ...
        if indicator_name.replace('RHNA_', '') in ['POPEMP_10', 'POPEMP_19', 'POPEMP_20', 'POPEMP_21', 'POPEMP_22', 'POPEMP_25', 'HSG_1', 'HSG_5', 'HSG_6'
                            , 'OVER_4', 'OVER_5', 'OVER_6', 'OVER_8', 'OVER_9', 'FARM_2', 'LGFEM_1', 'LGFEM_3', 'LGFEM_4', 'LGFEM_5'
                            , 'SEN_1', 'SEN_2', 'SEN_3', 'DISAB_3', 'HOMELS_1', 'HOMELS_2', 'HOMELS_3', 'HOMELS_4', 'ELI_2', 'AFFH_1', 'AFFH_2']:
...
def plot_rhna(export):
    ...
    if export:
        file_html = path_plots / f'{indicator_name}_.html'
        file_png  = path_plots / f'{indicator_name}_.png'
...
def export_rhna(df_prod, df_pct=None):
    ...
    indicator_name2 = indicator_name.replace('RHNA_', '')
    
    try:
        path_plot = path_plots / f'{indicator_name}_.png'
        img = Image(path_plot)
    except: pass
    ...
    if indicator_name2 not in wb.sheetnames:
        wb.create_sheet(indicator_name2)
    if indicator_name2 in wb.sheetnames:
        sheet_index = wb.sheetnames.index(indicator_name2)
        wb.remove(wb[indicator_name2])
        wb.create_sheet(indicator_name2, sheet_index)
    ws = wb[indicator_name2]
    ...
    df_title.iloc[0, 0] = f'{indicator_name2}: {title}'
    ...
    df_source.iloc[0, 1] = dict_about[source][indicator_name2]['Source'][0]

    df_years = pd.DataFrame([['' for _ in range(len(df_prod2.columns))]])
    df_years.iloc[0, 0] = 'Year(s)'
    df_years.iloc[0, 1] = dict_about[source][indicator_name2]['Year(s)'][0]

    df_notes = split_notes(dict_about[source][indicator_name2]['Notes'])
        ...
        if column == 'A':
            ...
            for cell in col:
                try:
                    ...
                    if int(cell_num) > row_num: # 6 (style 1)
