...
def popemp_2(dt_find_replace, df_prod, df_pct, year_max, year_min):
...
def hsg_1(dt_find_replace, df_prod, jurisdiction, county):
    ...
    if df_prod[ f'{jurisdiction} ({year_max})'].sum() > df_prod[ f'{jurisdiction} ({year_min})'].sum(): increased_or_decreased = 'increased'
    else: increased_or_decreased = 'decreased'
    if increased_or_decreased == 'increased' and prod_pct_diff > prod_pct_diff_county: above_or_below_county = 'above'
    else: above_or_below_county = 'below'
    if increased_or_decreased == 'increased' and prod_pct_diff > prod_pct_diff_region: above_or_below_region = 'above'
    else: above_or_below_region = 'below'
    ...
    if prod_pct_sfd > prod_pct_sfd_region: above_or_below_region_sfd = 'above'
    else: above_or_below_region_sfd = 'below'
...
def hsg_3(dt_find_replace, df_pct, jurisdiction):
...
def hsg_4(dt_find_replace, df_prod, df_pct):
    ...
    most_common_year_built = df_pct[df_pct['Percentage']==pct_most_common_year_built]['Year Built'].values[0].replace('Built ', '')
...
def hsg_7(dt_find_replace, df_pct, jurisdiction):
    ...
    most_common_home_value   = df_t[df_t['Pct']==pct_most_common_home_value]['Value'].values[0].replace('Units valued ', '')
    df_t_region = df_pct[df_pct['Geography']=='SACOG Region'].sum(axis=0, numeric_only=True).reset_index().rename(columns={'index':'Value', 0:'Pct'})
...
def eli_1(dt_find_replace, df_prod, jurisdiction):
...
## TODO: Acutely low income households
def eli_4(dt_find_replace):
    pass

## TODO: HCD/CTAC Opportunity Areas
def affh_2(dt_find_replace):
    pass
