...
def summarize_typology_jurisdiction(df):
    summaries = {}
    for juris, group in df.groupby(['COUNTY', 'JURIS']):
...
typ_overlay_merge = pd.merge(df_typol, gdf_overlay, on='GEOCODE')
clean_typ_overlay = typ_overlay_merge[['Countyname', 'GEOCODE', 'typ_cat', 'ohu_24', 'rhu_24', 'hh_24', 'JURIS', 'county', 'typ_simplified_name', 'typ_name']].rename(columns={'Countyname':'COUNTY'})
...
gdf_blocks_tract = gpd.sjoin(gdf_blocks, gdf_tracts[['GEOCODE','geometry']], how='left', predicate='within')
...
gdf_block_juris = gpd.overlay(gdf_blocks_tract, gdf_jurisdictions[['JURIS','geometry']], how='intersection')
...
clean_typ_overlay = clean_typ_overlay.merge(tract_juris_pop[['GEOCODE', 'JURIS', 'pop_scalar']], on=['GEOCODE', 'JURIS'], how='left')
...
df_jur_summaries = df_jur_summaries.reset_index().rename(columns={'level_0': 'County', 'level_1':'Jurisdiction','index': 'Typology'}).drop(columns=['level_2'])
...
df_county_summaries = df_county_summaries.reset_index().rename(columns={'level_0': 'County','index': 'Typology'}).drop(columns=['level_1'])
...
list_df = []
for county in dt.keys():
    df_county = df_jur_summaries[df_jur_summaries['County'] == county]
    jurisdictions = dt[county]
    for jurisdiction in jurisdictions:
        df_juris = df_county[df_county['Jurisdiction']==jurisdiction]
        list_df.append(df_juris)
df_jur_summaries = pd.concat(list_df)
