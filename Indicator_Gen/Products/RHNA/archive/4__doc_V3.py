...
def popemp_1(df_prod, year_min, year_max, dt_find_replace):
    ...
    if prod_pct_diff > 0: increased_or_decreased_pop = 'increased'
    else: increased_or_decreased_pop = 'decreased'
    ...
    if prod_pct_diff > prod_pct_diff_region: above_or_below_region = 'above'
    ...
    if prod_pct_diff_2020 > 0: increased_or_decreased_pop_2020 = 'increased'
    else: increased_or_decreased_pop_2020 = 'decreased'
...
def create_find_replace_dictionary(indicator, county, jurisdiction):
    ...
    if indicator == 'POPEMP_2':
        ...
        if prod_non_white_pct_diff > 0: increased_or_decreased_prod_non_white_pct = 'increased'
        else: increased_or_decreased_prod_non_white_pct = 'decreased'
        prod_white_pct_diff = abs(prod_year_max_white_pct - prod_year_min_white_pct)
        if prod_white_pct_diff > 0: increased_or_decreased_prod_white_pct = 'increased'
        else: increased_or_decreased_prod_white_pct = 'decreased'
    ...
    if indicator == 'POPEMP_4':
        ...
        if prod_year_max_under_18 > prod_year_min_under_18: increased_or_decreased_under_18 = 'increased'
        else: increased_or_decreased_under_18 = 'decreased'
        if prod_year_max_over_65 > prod_year_min_over_65: increased_or_decreased_over_65 = 'increased'
    ...
    if indicator == 'POPEMP_5':
        ...
        if prod_pct_moved > prod_pct_moved_region: more_or_less_region = 'more'
    ...
    if indicator == 'POPEMP_11':
        ...
        if prod_jobs_pct_diff > 0: increased_or_decreased_jobs = 'increased'
        else: increased_or_decreased_jobs = 'decreased'
    ...
    if indicator == 'POPEMP_13':
        prod_year_min = df_prod[df_prod['Year']==year_min][jurisdiction].values[0]
        prod_year_max = df_prod[df_prod['Year']==year_max][jurisdiction].values[0]
        if prod_year_max > prod_year_min: increased_or_decreased_ratio = 'increased'
        else: increased_or_decreased_ratio = 'decreased'
        if prod_year_max > 1: importer_or_exporter = 'importer'
        else: importer_or_exporter = 'exporter'
    ...
    if indicator == 'POPEMP_15':
        ...
        if prod_rate_diff > 0: increased_or_decreased_rate = 'increased'
        else: increased_or_decreased_rate = 'decreased'
        ...
        if prod_rate_diff_year_min > 0: increase_or_decrease_rate_year_min = 'increase'
        else: increase_or_decrease_rate_year_min = 'decrease'
    ...
    if indicator == 'POPEMP_16':
        ...
        if prod_total_rent > prod_total_own: more_or_less_renters = 'more'
        else: more_or_less_renters = 'less'
        ...
        pct_own_region  = df_pct[df_pct['Geography']==f'SACOG Region'   ]['Owner occupied' ].values[0]
    ...
    if indicator == 'POPEMP_20':
        try: pct_own_black    = df_pct[df_pct['Race/Ethnicity']=='Black or African American']['Owner occupied'].values[0]; dt_find_replace['[POPEMP_20_pct_own_black]'   ] = f'{pct_own_black:.1%}'
        except: pct_own_black=None
        try: pct_own_asian    = df_pct[df_pct['Race/Ethnicity']=='Asian'                    ]['Owner occupied'].values[0]; dt_find_replace['[POPEMP_20_pct_own_asian]'   ] = f'{pct_own_asian:.1%}'
        except: pct_own_asian=None
        try: pct_own_hispanic = df_pct[df_pct['Race/Ethnicity']=='Hispanic or Latino'       ]['Owner occupied'].values[0]; dt_find_replace['[POPEMP_20_pct_own_hispanic]'] = f'{pct_own_hispanic:.1%}'
        except: pct_own_hispanic=None
        try: pct_own_white    = df_pct[df_pct['Race/Ethnicity']=='White (NH)'               ]['Owner occupied'].values[0]; dt_find_replace['[POPEMP_20_pct_own_white]'   ] = f'{pct_own_white:.1%}'
        except: pct_own_white=None
    ...
    if indicator == 'HSG_1':
        # TODO:
        # HERE, update community snapshot
        year_min = 2010
        year_max = 2024
        prod_year_min_sfd = df_prod[df_prod['Housing Type'] == 'Single Family Detached'        ][f'Year {year_min}'].values[0]
        prod_year_min_sfa = df_prod[df_prod['Housing Type'] == 'Single Family Attached'        ][f'Year {year_min}'].values[0]
        prod_year_min_mfs = df_prod[df_prod['Housing Type'] == 'Multifamily: Two to Four Units'][f'Year {year_min}'].values[0]
        prod_year_min_mfl = df_prod[df_prod['Housing Type'] == 'Multifamily: 5+ Units'         ][f'Year {year_min}'].values[0]
        ...
        if prod_diff_sf_mf > 0: more_or_less_units = 'more'
        else: more_or_less_units = 'less'
        prod_year_max_sfd = df_prod[df_prod['Housing Type'] == 'Single Family Detached'        ][f'Year {year_max}'].values[0]
        prod_year_max_sfa = df_prod[df_prod['Housing Type'] == 'Single Family Attached'        ][f'Year {year_max}'].values[0]
        prod_year_max_mfs = df_prod[df_prod['Housing Type'] == 'Multifamily: Two to Four Units'][f'Year {year_max}'].values[0]
        prod_year_max_mfl = df_prod[df_prod['Housing Type'] == 'Multifamily: 5+ Units'         ][f'Year {year_max}'].values[0]
        prod_year_max_mh  = df_prod[df_prod['Housing Type'] == 'Mobile Homes'                  ][f'Year {year_max}'].values[0]
    ...
    if indicator == 'HSG_3':
        df_t = df_pct[df_pct['Geography']==jurisdiction].sum(axis=0, numeric_only=True).reset_index().rename(columns={'index':'Value', 0:'Pct'})
        pct_most_common_vacancy_type   = df_t['Pct'].max()
        most_common_vacancy_type   = df_t[df_t['Pct']==pct_most_common_vacancy_type]['Value'].values[0].lower()
        df_t_region = df_pct[df_pct['Geography']=='SACOG Region'].sum(axis=0, numeric_only=True).reset_index().rename(columns={'index':'Value', 0:'Pct'})
        ...
        most_common_vacancy_type_region_1 = df_t_region[df_t_region['Pct']==pct_most_common_vacancy_type_region_1]['Value'].values[0].lower()
        most_common_vacancy_type_region_2 = df_t_region[df_t_region['Pct']==pct_most_common_vacancy_type_region_2]['Value'].values[0].lower()
        most_common_vacancy_type_region_3 = df_t_region[df_t_region['Pct']==pct_most_common_vacancy_type_region_3]['Value'].values[0].lower()
    ...
    if indicator == 'HSG_4':
        pct_most_common_year_built = df_pct['Percentage'].max()
        most_common_year_built = df_pct[df_pct['Percentage']==pct_most_common_year_built]['Year Built'].values[0]
        pct_year_built_2010_or_later  = df_pct [df_pct ['Year Built']=='Built 2010 or later']['Percentage'].values[0]
    ...
    if indicator == 'HSG_6':
        try: pct_kitchen_rent  = df_pct[df_pct['Housing Issue']=='Lacking kitchen facilities' ]['Renter occupied'].values[0]; dt_find_replace['[HSG_6_pct_kitchen_rent]'  ] = f'{pct_kitchen_rent:.1%}'
        except: pct_kitchen_rent=None
        try: pct_kitchen_own   = df_pct[df_pct['Housing Issue']=='Lacking kitchen facilities' ]['Owner occupied' ].values[0]; dt_find_replace['[HSG_6_pct_kitchen_own]'   ] = f'{pct_kitchen_own:.1%}'
        except: pct_kitchen_own=None
        try: pct_plumbing_rent = df_pct[df_pct['Housing Issue']=='Lacking plumbing facilities']['Renter occupied'].values[0]; dt_find_replace['[HSG_6_pct_plumbing_rent]' ] = f'{pct_plumbing_rent:.1%}'
        except: pct_plumbing_rent=None
        ...
        except: pct_plumbing_own=None
        
    if indicator == 'HSG_7':
        df_t = df_pct[df_pct['Geography']==jurisdiction].sum(axis=0, numeric_only=True).reset_index().rename(columns={'index':'Value', 0:'Pct'})
        pct_most_common_home_value   = df_t['Pct'].max()
        most_common_home_value   = df_t[df_t['Pct']==pct_most_common_home_value]['Value'].values[0]
        df_t_region = df_pct[df_pct['Geography']=='SACOG Region'].sum(axis=0, numeric_only=True).reset_index().rename(columns={'index':'Value', 0:'Pct'})
        ...
        most_common_home_prod_region_1 = df_t_region[df_t_region['Pct']==pct_most_common_home_prod_region_1]['Value'].values[0].replace('Units valued ', '')
        most_common_home_prod_region_2 = df_t_region[df_t_region['Pct']==pct_most_common_home_prod_region_2]['Value'].values[0].replace('Units valued ', '')
    ...
    if indicator == 'HSG_8':
        ...
        if prod_year_max > prod_year_min: increased_or_decreased = 'increased'
        else: increased_or_decreased = 'decreased'
        ...
        if prod_home_price_inc > prod_home_price_inc_county: above_or_below_county = 'above'
        else: above_or_below_county = 'below'
        ...
        if prod_home_price_inc > prod_home_price_inc_region: above_or_below_region = 'above'
        else: above_or_below_region = 'below'
    ...
    if indicator == 'HSG_9':
        df_t = df_pct[df_pct['Geography']==jurisdiction].sum(axis=0, numeric_only=True).reset_index().rename(columns={'index':'Rent', 0:'Pct'})
        pct_most_common_rent_price   = df_t['Pct'].max()
        pct_most_common_rent_price_2 = df_t[df_t['Pct']<df_t['Pct'].max()]['Pct'].max()
        most_common_rent_price   = df_t[df_t['Pct']==pct_most_common_rent_price  ]['Rent'].values[0].replace('Rent ', '')
        most_common_rent_price_2 = df_t[df_t['Pct']==pct_most_common_rent_price_2]['Rent'].values[0].replace('Rent ', '')
        df_t_county = df_pct[df_pct['Geography']==f'{county} County'].sum(axis=0, numeric_only=True).reset_index().rename(columns={'index':'Rent', 0:'Pct'})
        pct_most_common_rent_price_county = df_t_county['Pct'].max()
        most_common_rent_price_county   = df_t_county[df_t_county['Pct']==pct_most_common_rent_price_county]['Rent'].values[0].replace('Rent ', '')
    ...
    if indicator == 'HSG_10':
        ...
        if prod_year_max > prod_year_min: increased_or_decreased_rent = 'increased'
        else: increased_or_decreased_rent = 'decreased'
        prod_year_min_county      = df_prod[df_prod['Year']==year_min][f'{county} County'].values[0]
        prod_year_max_county      = df_prod[df_prod['Year']==year_max][f'{county} County'].values[0]
        prod_rent_pct_diff_county = abs((prod_year_max_county/prod_year_min_county)-1)
        if prod_year_max_county > prod_year_min_county: increased_or_decreased_rent_county = 'increased'
        else: increased_or_decreased_rent_county = 'decreased'
        prod_year_min_region      = df_prod[df_prod['Year']==year_min]['SACOG Region'].values[0]
        prod_year_max_region      = df_prod[df_prod['Year']==year_max]['SACOG Region'].values[0]
        prod_rent_pct_diff_region = abs((prod_year_max_region/prod_year_min_region)-1)
        if prod_year_max_region > prod_year_min_region: increased_or_decreased_rent_region = 'increased'
        else: increased_or_decreased_rent_region = 'decreased'
    ...
    if indicator == 'HSG_11':
        try: prod_total = df_prod['Number of Permits'].sum(); dt_find_replace['[HSG_11_prod_total]'        ] = f'{prod_total:,.0f}'
        except: prod_total=None
        try: prod_above_mod_pct = df_prod[df_prod['Income Group']=='ABOVE MOD PERMITS']['Number of Permits'].values[0] / prod_total; dt_find_replace['[HSG_11_prod_above_mod_pct]'] = f'{prod_above_mod_pct:.1%}'
        except: prod_above_mod_pct=None
        try: prod_mod_pct       = df_prod[df_prod['Income Group']=='MOD PERMITS'      ]['Number of Permits'].values[0] / prod_total; dt_find_replace['[HSG_11_prod_mod_pct]'      ] = f'{prod_mod_pct:.1%}'
        except: prod_mod_pct=None
        try: prod_low_pct = df_prod[df_prod['Income Group'].isin(['LI PERMITS', 'VLI PERMITS'])]['Number of Permits'].sum() / prod_total; dt_find_replace['[HSG_11_prod_low_pct]'      ] = f'{prod_low_pct:.1%}'
        except: prod_low_pct=None
        
    if indicator == 'OVER_1':
        pct_rent_more_than_15 = df_pct[df_pct['Tenure']=='Renter occupied']['More than 1.5 occupants per room'].values[0]
        pct_own_more_than_15  = df_pct[df_pct['Tenure']=='Owner occupied' ]['More than 1.5 occupants per room'].values[0]
        pct_rent_1_to_15      = df_pct[df_pct['Tenure']=='Renter occupied']['1.01 to 1.5 occupants per room'  ].values[0]
        pct_own_1_to_15       = df_pct[df_pct['Tenure']=='Owner occupied' ]['1.01 to 1.5 occupants per room'  ].values[0]
    ...
    if indicator == 'OVER_6':
        ...
        pct_50_plus_rent = df_pct[df_pct['Housing Tenure'] == 'Renter']['50% or more of income used for housing'].values[0]
        pct_50_plus_own  = df_pct[df_pct['Housing Tenure'] == 'Owner' ]['50% or more of income used for housing'].values[0]
    ...
    if indicator == 'OVER_9':
        try: pct_large_family_30_50   = df_pct[df_pct['Household Size']=='Large family with 5+ persons']['30%-50% of income used for housing'].values[0]; dt_find_replace['[OVER_9_pct_large_family_30_50]'  ] = f'{pct_large_family_30_50:.1%}'
        except: pct_large_family_30_50=None
        ...
        except: pct_large_family_50_plus=None
    ...
    if indicator == 'FARM_1':
        ...
        try: prod_pct_diff = abs(prod_year_max/prod_year_min-1); dt_find_replace['[FARM_1_prod_pct_diff]'] = f'{prod_pct_diff:.1%}'
        except: prod_pct_diff=None
        if prod_pct_diff > 1: increase_or_decrease = 'increase'
        else: increase_or_decrease = 'decrease'
        if prod_pct_diff is None: increase_or_decrease=None
        ...
        prod_pct_diff_region = abs(prod_year_max_region/prod_year_min_region-1)
        if prod_pct_diff_region > 1: increase_or_decrease_region = 'increase'
        else: increase_or_decrease_region = 'decrease'
    ...
    if indicator == 'FARM_2':
        ...
        if prod_year_max_permanent > prod_year_min_permanent: increased_or_decreased_permanent = 'increased'
        else: increased_or_decreased_permanent = 'decreased'
        if prod_year_max_seasonal > prod_year_min_seasonal: increased_or_decreased_seasonal = 'increased'
        else: increased_or_decreased_seasonal = 'decreased'
    ...
    if indicator == 'LGFEM_3':
        try: pct_large_family_below_50_ami = df_pct[df_pct['Household Size']=='Large family with 5+ persons']['0%-30% of AMI'].values[0] + df_pct[df_pct['Household Size']=='Large family with 5+ persons']['31%-50% of AMI'].values[0]; dt_find_replace['[LGFEM_3_pct_large_family_below_50_ami]'] = f'{pct_large_family_below_50_ami:.1%}'
        except: pct_large_family_below_50_ami=None
    ...
    if indicator == 'LGFEM_5':
        try: pct_w_children_below_fpl  = df_pct[df_pct['Family Status']=='Female-headed households with children'   ]['Below poverty level'].values[0]; dt_find_replace['[LGFEM_5_pct_w_children_below_fpl]' ] = f'{pct_w_children_below_fpl:.1%}'
        except: pct_w_children_below_fpl=None
        try: pct_no_children_below_fpl = df_pct[df_pct['Family Status']=='Female-headed households without children']['Below poverty level'].values[0]; dt_find_replace['[LGFEM_5_pct_no_children_below_fpl]'] = f'{pct_no_children_below_fpl:.1%}'
        except: pct_no_children_below_fpl=None

    if indicator == 'SEN_1':
        most_common_income_rent = df_pct[df_pct['Renter occupied'] == df_pct['Renter occupied'].max()]['Income Level'].values[0]
        most_common_income_own  = df_pct[df_pct['Owner occupied' ] == df_pct['Owner occupied' ].max()]['Income Level'].values[0]
        dt_find_replace['[SEN_1_most_common_income_rent]'] = most_common_income_rent
        dt_find_replace['[SEN_1_most_common_income_own]' ] = most_common_income_own#.replace('G', 'g')
    ...
    if indicator == 'SEN_3':
        try: pct_below_30_ami_50_plus      = df_pct[df_pct['Income Level']=='0%-30% of AMI'           ]['50%+ of income used for housing'  ].values[0]; dt_find_replace['[SEN_3_pct_below_30_ami_50_plus]'      ] = f'{pct_below_30_ami_50_plus:.1%}'
        except: pct_below_30_ami_50_plus=None
        try: pct_over_100_ami_less_than_30 = df_pct[df_pct['Income Level']=='Greater than 100% of AMI']['0%-30% of income used for housing'].values[0]; dt_find_replace['[SEN_3_pct_over_100_ami_less_than_30]' ] = f'{pct_over_100_ami_less_than_30:.1%}'
        except: pct_over_100_ami_less_than_30=None        
    ...
    if indicator == 'DISAB_4':
        prod_under_18_pct = df_prod[df_prod['Age Group']=='Under 18']['Population with Disabilities'].values[0] / df_prod['Population with Disabilities'].sum()
        prod_over_18_pct  = df_prod[df_prod['Age Group']=='18+'     ]['Population with Disabilities'].values[0] / df_prod['Population with Disabilities'].sum()
    ...
    if indicator == 'DISAB_5':
        most_common_residence_type = df_prod[df_prod['Population with Disabilities'] == df_prod['Population with Disabilities'].max()]['Residence Type'].values[0]
    ...
    if indicator == 'HOMELS_1':
        ...
        # dt_find_replace['[HOMELS_1_prod_w_children_unsheltered_pct]'] = f'{prod_w_children_unsheltered_pct:.1%}'

    if indicator == 'HOMELS_2':
        prod_homeless = df_prod['Homeless population'].sum()
        most_common_ethnicity_homeless = df_pct[df_pct['Share of homeless population'] == df_pct['Share of homeless population'].max()]['Race/Ethnicity'].values[0]
        pct_most_common_ethnicity_homeless = df_pct[df_pct['Race/Ethnicity']==most_common_ethnicity_homeless]['Share of homeless population'].values[0]
        pct_most_common_ethnicity_overall  = df_pct[df_pct['Race/Ethnicity']==most_common_ethnicity_homeless]['Share of overall population' ].values[0]
    ...
    if indicator == 'HOMELS_4':
        ...
        prod_year_min = df_prod[df_prod['Geography']==jurisdiction]['2020-21'].values[0]
        prod_year_max = df_prod[df_prod['Geography']==jurisdiction]['2023-24'].values[0]
        try: prod_pct_diff = abs(prod_year_max/prod_year_min-1); dt_find_replace['[HOMELS_4_prod_pct_diff]'       ] = f'{prod_pct_diff:.1%}'
        except: prod_pct_diff=None
        if prod_pct_diff > 1: increased_or_decreased = 'increase'
        else: increased_or_decreased = 'decrease'
        if prod_pct_diff is None: increased_or_decreased=None
        ...
        if prod_pct_diff_county > 1: increase_or_decrease_county = 'increase'
        else: increase_or_decrease_county = 'decrease'
        prod_year_min_region = df_prod[df_prod['Geography']=='SACOG Region']['2020-21'].values[0]
        prod_year_max_region = df_prod[df_prod['Geography']=='SACOG Region']['2023-24'].values[0]
        prod_pct_diff_region = abs(prod_year_max_region/prod_year_min_region-1)
        if prod_pct_diff_region > 1: increased_or_decreased_region = 'increase'
        else: increased_or_decreased_region = 'decrease'
    ...
    if indicator == 'AFFH_3':
        ...
        if pct_limited_english > pct_limited_english_region: above_or_below = 'above'
        else: above_or_below = 'below'
