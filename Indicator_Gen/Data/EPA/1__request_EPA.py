...
if source == 'CAM':
    ...
    indicator_name = 'Emissions_1'
    root_   = 'https://api.epa.gov/easey/emissions-mgmt/'
    email_  = 'tenoru@sacog.org'
    key_ = api_key
    data_   = 'emissions'
    geo_    = 'facility'
    params  = ['SO2', 'NOx']
    years   = sequence(year_min, year_max, 1) # 1990 earliest year
    geos    = ['Counties', 'States', 'National']

    # Additional code for CAM would go here
    print('Geographies available:')
    print();print()
    print(geos)
    print();print()
    input_geo = input('Which geography do you want to pull data for?')
    print();print()
    print('Years available:')
    print(years)
    print();print()
    input_years = input('Do you want to pull data for all years available? Select Yes/No:')
    if input_years.isnumeric():
