...
def weighted_rollup(group):

    weights = group['Weight']
    total_weight = weights.sum()

    # Weighted mean
    weighted_mean = np.average(group['Total'], weights=weights)

    # Correct weighted MOE
    a = weights / total_weight
    weighted_moe = np.sqrt(np.sum((a**2) * (group['ME']**2)))
...
def acs_processing_3(df_census, estimate, sample_type, indicator, geography, percentages, margin_of_error, MOE_thresh, num_vars, weighted_by, project, export_loc, metric, unincorporated, mpo):
    ...
    if margin_of_error == 'Yes':
        ...
        if weighted_by != '':
            df_census = df_census.groupby(geo_ID + ['Year', 'Race_Ethnicity', 'Variable'], as_index=False, sort=False).apply(weighted_rollup).reset_index(drop=True)
            df_census = calculate_ME_ratio(df_census, MOE_thresh)

            if geography == 'Counties':
                if mpo == 'Yes':
                    df_mpo = df_census.groupby(['State FIPS', 'MPO', 'Year', 'Race_Ethnicity', 'Variable'], as_index=False, sort=False).apply(weighted_rollup).reset_index(drop=True)
            ...
            if geography == 'Places':
                if unincorporated == 'Yes':
                    if indicator == 'Income_1':
                        ...
                        df_inc1 = df_census.groupby(['County Name', 'Year', 'Race_Ethnicity', 'Variable'], as_index=False, sort=False).apply(weighted_rollup).reset_index(drop=True)
                        ...
                        df_inc1 = df_census.groupby(['State FIPS', 'County Name', 'Year', 'Race_Ethnicity', 'Variable'], as_index=False, sort=False).apply(weighted_rollup).reset_index(drop=True)
                        df_counties = pd.read_excel(file_counties, sheet_name='Counties')
