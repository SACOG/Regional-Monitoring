...
def process_data_group(df: pd.DataFrame, data_group: str) -> pd.DataFrame:
    ...
    if data_group.lower() == 'race':
        df_copy['Race'] = df_copy['label'].apply(lambda x: x.split('!!')[-1] if pd.notnull(x) else None)
    elif data_group.lower() == 'age':
        df_copy['Age'] = df_copy['label'].apply(lambda x: x.split('!!')[-1] if pd.notnull(x) and ('Male' in x or 'Female' in x) else (x.split('!!')[-1] if any(str(i).isdigit() for i in x.split('!!')[-1]) else None) if pd.notnull(x) else None)
    elif data_group.lower() == 'gender':
        df_copy['Gender'] = df_copy['label'].apply(lambda x: 'Male' if 'Male' in x else ('Female' if 'Female' in x else None) if pd.notnull(x) else None)
    elif data_group.lower() == 'household income by race':
        df_copy['Race'] = df_copy['concept'].apply(lambda x: x.split('(')[-1].split(')')[0].replace(' HOUSEHOLDER', '') if pd.notnull(x) else None)
...
def dc_var_gen(df: Optional[pd.DataFrame] = None, 
              ...
              data_group: Optional[str] = None) -> Optional[pd.DataFrame]:


    """Generates a dataframe based on the desired variables.
    
    - Parameters:
        - df: dataframe generated by retreiving all json variables. 
        - csv_path: path to csv containing variables desired. 
        - variable_list: a list of variables defined 
             -If no variable list or csv is provided, the user will be prompted to provide a variable list which can be delimited
             by semicolon, pipe, or comma
        data_group:
            - One of:
                - Race
                - Age 
            ...
            - If none of the above are provided, a dataframe will be generated with a column named 'No Data Group' and an error message will be printed.    
    """
    ...
    if df is None and csv_path:
        try:
            ...
            return None
    if df is None:
        print("Variables dataframe empty. Exiting.")
        return None
    if variable_list:
